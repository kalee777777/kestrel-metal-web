...
| SITE-01 | 站内 schema 联系信息对齐 | ✅ 2026-09-22 部分完成——schema 邮箱已全站对齐为 `kalee@`(主),`sales@`/`info@`/`privacy@` 均已替换(`perf-scripts/static-jsonld.js` + `js/seo-enhance.js` + 全站 HTML)。**剩余**:电话格式 `+86-17832383339` → `+86 17832383339`、员工 60-100 → 51-100,随下次 schema 重建/部署一并执行(数字主体不变,不紧急) | AI | 🔄 |
...
| GEO-10 | GEO 自动化流水线部署 + 一次性配置 | 🔄 2026-09-25 上线并端到端验证——全站审计 215/215 页(平均 64,高 32/中 112/低 71)、20 个补丁待审、geo-faq 首跑 3 条待审 FAQ、全部路由通;新文章 Copper Wire Mesh(SEO 81/GEO 82)+ llms.txt 自动追加零人工验证。**PR 通道已全自动闭环**(GitHub Actions 免 PAT;仓库已开启 Actions 开 PR 权限):批准补丁 → 工作流(每 6h/手动/触发文件)自动开 PR → 人工合并即部署;首例 PR #1 已合并并线上验证(services.html 42→47 分,citation 维度 +30 定义句生效;fact_density 仅 7 为该页短板,需内容级补强而非补丁块)。**剩余**:全部完成——19 补丁经 PR #2 合并上线(20 页均分 44→52,4 处 AI 幻觉事实已修正);FAQ 服务端化收尾:6 条种子经 /api/faq/import 入 KV + 3 条 geo-faq 自动 FAQ 启用,faq.html 运行时注入 25 条(16 静态 + 9 KV)且 FAQPage schema 同步 25 条(2026-09-25 验证)。注:浏览器 localStorage 经 LevelDB 取证仅含种子级数据,无独有内容 | AI + 人工 | 🔄 |
